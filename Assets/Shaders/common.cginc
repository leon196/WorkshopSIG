...
void moda(inout float2 p, float repetitions) {
  float angle = TAU/repetitions;
  float a = atan2(p.y, p.x) + angle/2.;
  a = fmod(abs(a),angle) - angle/2.;
...
float sdbox (float3 p, float2 b) { float2 d = abs(p.xz) - b; return min(max(d.x,d.y),0.0) + length(max(d,0.0)); }
float sdiso (float3 p, float r) { return dot(p, normalize(sign(p)))-r; }
...
float fbm3 (float3 p, float t, float3 offset) {
    float value = 0.0;
    float amplitud = .5;
    for (float i = 3.; i >= 1.; --i) {
...
void rot (inout float2 p, float a) { float c=cos(a),s=sin(a); p = mul(float2x2(c,-s,s,c), p); }
float amod (inout float2 p, float count) { float an = TAU/count; float a = atan2(p.y,p.x)+an/2.; float c = floor(a/an); c = lerp(c,abs(c),step(count*.5,abs(c))); a = fmod(abs(a),an)-an/2.; p.xy = float2(cos(a),sin(a))*length(p); return c; }